...
function navigateTo(page, isBack = false) {
    ...
    const navPage = page === 'expenses'
        ? 'dashboard'
        : (page === 'group-detail' || page === 'group-design')
        ? 'groups'
    ...
    if (fab) fab.style.display = (page === 'expenses' || page === 'settings' || page === 'detail' || page === 'trash' || page === 'categories' || page === 'groups' || page === 'group-detail' || page === 'group-design') ? 'none' : '';
...
async function saveNewAccount(type) {
    ...
    if (window.appState.isDemo) {
        ...
        try {
            ...
            const id = await addAccountToDB(payload);
            if (id) { if (typeof markAccountAsJustAdded === 'function') markAccountAsJustAdded(id); if (typeof recordAddQuickFormHistory === 'function') recordAddQuickFormHistory(); closeModal(); showToast(`Đã thêm "${name}"`, 'success'); }
        } catch (error) {
