...
describe('mobile Settings update section', () => {
  it('renders version and Check button in Settings', () => {
    ...
    expect(pageContent.innerHTML).toContain('Ting! v1.3.2');
  ...
  it('shows Update button when Android update is available', () => {
    const { exports } = loadMobileUi({
      appState: {
        appVersion: '1.3.2',
        updateStatus: { status: 'update-available', info: { latestVersion: '1.3.3', releaseNotes: 'Fix' } },
        updateLog: [],
        settings: { theme: 'system' },
        trashAccounts: [],
        customCategories: [],
    ...
    expect(html).toContain('Phiên bản mới 1.3.3');
